...
bool LoadTextureFromResource(int resId, ID3D11ShaderResourceView** out_srv, int* out_width, int* out_height)
{
    ...
    int            w = 0, h = 0;
    unsigned char* pixels = stbi_load_from_memory((const stbi_uc*)data, size, &w, &h, nullptr, 4);
...
int main(int, char**)
{
    ...
    if (!io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\segoeui.ttf", fontSize * dpi))
    ...
    int logoW = 0, logoH = 0;
    if (LoadTextureFromResource(IDR_LOGO_PNG, &logoSrv, &logoW, &logoH))
        app.SetLogo(logoSrv, logoW, logoH);
